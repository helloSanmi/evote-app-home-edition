...
export default function Results() {
  ...
  async function loadAll() {
    try {
      ...
      let pid = Number(periodId);
      if (!pid) {
        // fallback: get user’s sessions, pick the latest published one
        const prt = await fetch(api("/api/user/participated"), { headers: auth }).then(safeJson);
        const arr = Array.isArray(prt) ? prt : [];
        const published = arr.filter(s => s.resultsPublished);
        if (published.length) pid = published[0].id;
      }

      if (!pid) {
        setPeriod(null); setList([]); setYouVoted(null); setWinnerIds([]);
        setLoading(false);
        return;
      }

      // self vote (even before publish)
      const sr = await fetch(api(`/api/vote/status?periodId=${pid}`), { headers: auth }).then(safeJson);
      setYouVoted(sr?.youVoted || null);

      const rr = await fetch(api(`/api/public/results?periodId=${pid}`), { headers: auth });
      if (!rr.ok) {
        const err = await safeJson(rr);
        setPeriod(null); setList([]); setWinnerIds([]);
        if (rr.status === 403 && err?.error) {
    ...
    <div className="max-w-6xl mx-auto px-4">
      {!period ? (
        ...
        <div className="bg-white rounded-xl shadow p-6 mt-6 transition hover:shadow-lg">
          ...
          {list.length === 0 ? (
            <div className="text-gray-500">You’re not eligible to view results for this session or there are no results.</div>
          ) : (
            <>
              {/* Winner section */}
                  ...
                  {list
                    ...
                    .map((c) => (
                      <div key={c.id} className="flex items-center gap-3">
                        <img src={c.photoUrl ? api(c.photoUrl) : "/placeholder.png"} className="w-10 h-10 rounded object-cover" alt={c.name} />
                ...
                {list.map((c) => (
                  ...
                  >
                    <img
                      ...
                      alt={c.name}
                      onClick={() => c.photoUrl && setImg(api(c.photoUrl))}
                    />
                    <div className="font-semibold">{c.name}</div>
                    <div className="text-sm text-gray-600">{c.state || "-"} • {c.lga || "-"}</div>
